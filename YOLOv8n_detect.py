...
# ⭐ NEW: Speed controls (important for Raspberry Pi)
parser.add_argument('--imgsz', help='YOLO inference image size (e.g. 256, 320, 416). Smaller = faster.', default=256)
parser.add_argument('--interval', help='Run YOLO every N seconds (e.g. 0.4). Camera stays smooth between runs.', default=0.5)
...
if user_res:
    ...
    # Picamera2 config needs a size, so give a safe default
...
avg_frame_rate = 0
frame_rate_buffer = []
fps_avg_len = 60  # faster reacting average
img_count = 0

# ⭐ NEW: store last detections so camera stays smooth
...
while True:
    ...
    # ---- Resize for display (and consistent compute) ----
    if resize:
        frame = cv2.resize(frame, (resW, resH))

    # -----------------------------
    # ✅ SPEED FIX: YOLO only every DETECTION_INTERVAL seconds
    # -----------------------------
    now = time.time()
    if (now - last_detection_time) >= DETECTION_INTERVAL:
        infer_start = time.perf_counter()

        # Use predict with imgsz + conf (faster control)
        results = model.predict(frame, imgsz=IMG_SZ, conf=min_thresh, verbose=False)

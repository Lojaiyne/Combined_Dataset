    ...
    while True:
        ...
        for i in range(len(detections)):
            ...
            if conf > min_thresh:
                ...
                # Optional: ignore tiny boxes
                bbox_area = (xmax - xmin) * (ymax - ymin)
                if args.min_area and bbox_area < args.min_area:
                    continue

                # Fire only when center crosses trigger line
                cx = int((xmin + xmax) / 2)
                if classname in solenoid_map and cx >= trigger_line_x:
                    print(f"FIRE {classname} at x={cx}")
        ...
        # Draw motor status
        motor_status = "ON" if args.motor_enable else "OFF"
